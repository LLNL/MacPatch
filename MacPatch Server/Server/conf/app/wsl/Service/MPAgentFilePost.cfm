...
<cfif NOT IsDefined("form.requestID") OR NOT IsDefined("form.userID") OR NOT IsDefined("form.token")>
		<cfset response.errorno = "1" />
		<cfset response.errormsg = "Error: Invalid Format." />
		<cfoutput>#Trim(SerializeJSON(response))#</cfoutput>
	<cfabort>
<cfelse>
	<cfif NOT isValidRequest(form.requestID, form.userID)>
...
<!--- Upload the Agent pkg/zip file --->
<cfset pkgs = ArrayNew(1)> 
<cfif IsDefined("form.fComplete")>
	<cffile action = "upload"
	      	fileField="form.fComplete"
	      	Destination="#ulTmpPath#"
	      	nameConflict="overwrite"
	        mode="644" result="pkg1">
	<cfset aap = #ArrayAppend(pkgs, pkg1.clientFile)#>		
</cfif>
<cfif IsDefined("form.fBase")>		
	<cffile action = "upload"
	      	fileField="form.fBase"
	      	Destination="#ulTmpPath#"
	      	nameConflict="overwrite"
	        mode="644" result="pkg2">
	<cfset aap = #ArrayAppend(pkgs, pkg2.clientFile)#>
</cfif>
<cfif IsDefined("form.fUpdate")>		
	<cffile action = "upload"
	      	fileField="form.fUpdate"
	      	Destination="#ulTmpPath#"
	      	nameConflict="overwrite"
	        mode="644" result="pkg3">		
	<cfset aap = #ArrayAppend(pkgs, pkg3.clientFile)#>
</cfif>

<!--- Move Packages To New Location --->
<cfset respObj.agent.hash = #pkgUUID# />
<cfset respObj.updater.hash = #pkgUUID# />
