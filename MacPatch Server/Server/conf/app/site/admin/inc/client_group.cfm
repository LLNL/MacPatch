	...
	var eventsMenu = {
		bindings: {
			'HardwareOverview': function(t) {
				popUpInvWindow('client_group_inv_details.cfm?type=hardwareoverview&cuuid='+t.id+'','Client Info');
			},
			'SoftwareOverview': function(t) {
				popUpInvWindow('client_group_inv_details.cfm?type=systemoverview&cuuid='+t.id+'','Client Info');
			},
			'NetworkOverview': function(t) {
				popUpInvWindow('client_group_inv_details.cfm?type=networkoverview&cuuid='+t.id+'','Client Info');
			},
			'Applications': function(t) {
				popUpInvWindow('client_group_inv_details.cfm?type=applications&cuuid='+t.id+'','Client Info');
			},
			'ApplicationUsage': function(t) {
				popUpInvWindow('client_group_inv_details.cfm?type=applicationusage&cuuid='+t.id+'','Client Info');
			},
			'DirectoryService': function(t) {
				popUpInvWindow('client_group_inv_details.cfm?type=directoryoverview&cuuid='+t.id+'','Client Info');
			},
			'Frameworks': function(t) {
				popUpInvWindow('client_group_inv_details.cfm?type=frameworks&cuuid='+t.id+'','Client Info');
			},
			'InternetPlugins': function(t) {
				popUpInvWindow('client_group_inv_details.cfm?type=internetplugins&cuuid='+t.id+'','Client Info');
			},
			'ClientTasks': function(t) {
				popUpInvWindow('client_group_inv_details.cfm?type=clienttasks&cuuid='+t.id+'','Client Info');
			},
			'DiskInfo': function(t) {
				popUpInvWindow('client_group_inv_details.cfm?type=diskinfo&cuuid='+t.id+'','Client Info');
			},
			...
			var mygrid = $("#list").jqGrid(
			{
				...
				loadComplete: function(){
					...
					for(var i=0;i<ids.length;i++){
						...
						av = "<input type='image' style='padding-left:4px;' onclick=popUpWindow('client_group_avinfo_details.cfm?cuuid="+cl+"'); src='/admin/images/"+avIcon+"' height='16' width='16' align='texttop' TITLE='AV Properties'>";
