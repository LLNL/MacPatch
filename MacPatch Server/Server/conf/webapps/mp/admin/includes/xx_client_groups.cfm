...
<script src="./_assets/js/jquery/test/jquery.contextmenu.r2.js" type="text/javascript"></script>
	...
	$(document).ready(function()
		{
			...
			$("#list").jqGrid(
			{
				url:'./includes/xx_client_groups.cfc?method=getClientsForGroup&clientgroup=<cfoutput>#ClientGroupName#</cfoutput>', //CFC that will return the users
				datatype: 'json', //We specify that the datatype we will be using will be JSON
				ajaxGridOptions:{
					type: "POST",
					url: "./includes/xx_client_groups.cfc?method=getClientsForGroup&clientgroup=<cfoutput>#ClientGroupName#</cfoutput>",
				},
				<cfif isDefined("fID")>
				postData: {
					/* method: '', */
				...
				multiselect: true,
				multiboxonly: true,
				editurl:"./includes/xx_client_groups.cfc?method=editClientsForGroup",//Not used right now.
			...
			*/
			...
			$('#list').jqGrid ('navButtonAdd', '#pager', {
				...
				onClickButton:  function () {
					window.location.href = './includes/xx_client_groups_export.cfm?clientgroup=<cfoutput>#ClientGroupName#</cfoutput>';
				}
			});
		}
	);	
