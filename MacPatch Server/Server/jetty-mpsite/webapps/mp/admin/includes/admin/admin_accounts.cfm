	...
	$(document).ready(function()
		{
			...
			.navButtonAdd('#pager',{
			   ...
			   onClickButton: function(){ 
          		...
          		if (rowid) { 
					load('./index.cfm?adm_mp_accounts&action=<cfoutput>#encrypt('edit',session.usrKey,'AES/CBC/PKCS5Padding','base64')#</cfoutput>&u='+rowid);
          		}
			   }, 
			   position:"last"
			});
...
<cfsavecontent variable="newConfig">
  <nocfml><cfoutput>
    ...
    <cfform action="index.cfm?adm_mp_accounts&action=#encrypt('submitAdd',session.usrKey,'AES/CBC/PKCS5Padding','base64')#" method="Post" name="AddNewUser">
		<fieldset>
		...
		<fieldset>
			<table>
			<tr><td>
				<input class="button medium gray" type="button" value="Cancel" onclick="load('./index.cfm?adm_mp_accounts');">
...
<cfsavecontent variable="editConfig">
  <nocfml><cfoutput>
    ...
    <cfform action="index.cfm?adm_mp_accounts&action=#encrypt('submitEdit',session.usrKey,'AES/CBC/PKCS5Padding','base64')#" method="Post" name="eusr">
		<fieldset>
...
<cfif session.adm_mp_accounts EQ "2">
	...
	<cfif accountData.RecordCount EQ "1">
		<cfif accountData.user_type EQ "0">
			...
			<cflocation url="#session.cflocFix#/admin/index.cfm?adm_mp_accounts">
			<cfabort>
		</cfif>
	</cfif>
	...
	<cfif form.USER_PASS0 NEQ "">
		<cfif accountData.RecordCount EQ "1">
			<cfif accountData.user_pass NEQ #hash(form.USER_PASS0,'MD5')#>
				...
				<cflocation url="#session.cflocFix#/admin/index.cfm?adm_mp_accounts&action=#encrypt('edit',session.usrKey,'AES/CBC/PKCS5Padding','base64')#&u=#form.rid#">
			<cfelse>
				<cfset updatePass = "1">	
			</cfif>
		</cfif>
