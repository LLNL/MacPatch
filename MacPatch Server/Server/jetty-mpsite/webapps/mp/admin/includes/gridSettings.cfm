...
<cfset jqGridInfoData = #Decrypt(DATA,session.usrKey)#>

<cfset _xmlConfDir = #Expandpath("./includes/settings")#>
...
<h3>Edit Columns for <cfoutput>#jqGridInfoData#</cfoutput></h3>
<form name="gridSettings" method="post">
    ...
    <tr>
        ...
        <td align="right"><input name="button1" type="submit" value="Reset" />(Reset view to default view.)</td>
    </tr>
    </table>
	...
	<tr>
	<cfoutput>
		<!--- <td><input type="text" name="order" size="2" value="#x.order#"></td> --->
	...
	<a href="<cfoutput>index.cfm?#session.pasRef#</cfoutput>"><input type="button" name="cancel" value="Cancel" /></a>
</form>
...
<cfif IsDefined("form") AND StructIsempty(form) EQ false>
	<cfif form.submit EQ "Reset">
        ...
        <cflocation url="#CGI.HTTP_ORIGIN#/admin/index.cfm?#session.curRef#">
        <cfabort>
	</cfif>
    ...
    <!------>
	<cflocation url="#CGI.HTTP_ORIGIN#/admin/index.cfm?#session.pasRef#">
</cfif>
