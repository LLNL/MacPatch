<cfcomponent>
	...
	<cffunction name="init" returntype="update_patch_group" output="no">
		<cfreturn this>
	</cffunction>
    ...
    <cffunction name="GetPatchGroupPatchesExtended" access="public" returntype="any" output="no">
		...
		<cfquery datasource="#this.ds#" name="qGetBaselineID" maxrows="1">
        	Select baseline_id from mp_baseline
            Where state = '1'
            Order by mdate Desc
        </cfquery>
		<cfif qGetBaselineID.RecordCount EQ 1>
        	<cfset bID = qGetBaselineID.baseline_id>
        <cfelse>
        	<cfset bID = "0">
        </cfif>
        ...
        <cftry>
            <cfquery datasource="#this.ds#" name="qGetUpdatesApple">
				...
				( SELECT 1
					...
					WHERE bl.p_id = mpgp.patch_id AND bl.baseline_id = '#bID#')
				THEN "1" ELSE "0"
                END AS baseline
                ...
                <array>
                    ...
                    <dict>
                        ...
                        <array>
                            ...
                            <cfset patchRes = GetPatchInfo(item,bID) />
...
<!--- #################################################### --->
    ...
    <cffunction name="GetPatchInfo" access="public" returntype="query" output="no">
    	<cfargument name="id" required="yes">
        <cfargument name="bid" required="yes">
        ...
        <cfquery datasource="#this.ds#" name="qGetPatchInfo" cachedwithin="#CreateTimeSpan(0, 0, 0, 30)#">
            ...
            CASE WHEN EXISTS
				( SELECT 1
					FROM mp_baseline_patches bl
					WHERE bl.p_id = mpp.puuid AND bl.baseline_id = '#bid#')
				THEN "1" ELSE "0"
                END AS baseline
            from mp_patches mpp
