...
<cfif IsDefined("form.mainPackage") AND isEmpty(form.mainPackage)>
    ...
    <cfset BaseSwDistPath = "/Library/MacPatch/Content/Web/sw">
    ...
    <!--- Upload the software --->
    <cffile action="upload" fileField="form.mainPackage" destination="#SwDistDir#" nameconflict="overwrite" />
    <cfset theFilePath = #BaseSwDistPath# & "/" & #form.suuid# & "/" & #clientfile#>
    <cfset _sw_url = "/sw/" & #form.suuid# & "/" & #clientfile#>
    <cfset _sw_sizeK = #fileSize# / 1024>
...
<cfquery name="qInsert1" datasource="#session.dbsource#" result="res">
    ...
    Set 
        ...
        <cfif IsDefined("_sw_sizeK")>
        	,sw_size = <cfqueryparam value="#_sw_sizeK#">
        </cfif>
