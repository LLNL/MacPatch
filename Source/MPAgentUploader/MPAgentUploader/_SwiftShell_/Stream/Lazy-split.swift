...
extension Collection where Iterator.Element: Equatable {
	...
	func splitOnce(separator: Iterator.Element) -> (head: SubSequence, tail: SubSequence?) {
		guard let nextindex = firstIndex(of: separator) else { return (self[startIndex..<endIndex], nil) }
...
public struct PartialSourceLazySplitSequence <Base: Collection>: IteratorProtocol, LazySequenceProtocol where
	...
	private var gs: LazyMapSequence<<#Base: Sequence#>, Any>.Iterator<AnyIterator<Base>, LazySplitSequence<Base>>
