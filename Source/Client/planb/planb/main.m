...
NSURL* URLForPackagePath(NSString *pkg) {
  ...
  static NSString *track;
	...
	dispatch_once(&onceToken, ^{
  	...
  	NSString *fileName = [NSString stringWithFormat:@"%@-%@.dmg", pkg, track];
	NSString *urlPath = [kBaseURL stringByAppendingPathComponent:fileName];
	NSURL *result = [NSURL URLWithString:urlPath];
	return result;
}
...
int main(int argc, const char **argv) {
	@autoreleasepool {
		...
		MOLAuthenticatingURLSession *authURLSession = [[MOLAuthenticatingURLSession alloc] initWithSessionConfiguration:config];
