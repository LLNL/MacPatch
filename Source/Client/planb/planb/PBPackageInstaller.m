...
- (instancetype)initWithURL:(NSURL *)packageURL receiptName:(NSString *)receipt
{
	self = [super init];
...
- (void)dealloc
{
	if (self.mountPoint)
	{
    	...
    	if (err) {
      		[self log:@"Error: could not delete temporary mount point %@: %@", self.mountPoint, err.localizedDescription];
    	}
  	}
}
...
- (BOOL)install
{
	...
	NSString *newPath = [[path stringByDeletingLastPathComponent] stringByAppendingPathComponent:self.packageURL.lastPathComponent];
...
- (NSString *)generateMountPoint
{
	...
	return [[NSFileManager defaultManager] stringWithFileSystemRepresentation:tmpdir length:strlen(tmpdir)];
}
...
- (NSString *)downloadPackage
{
	...
	for (NSUInteger i = 1; i <= self.downloadAttemptsMax; ++i)
	{
		...
		dispatch_semaphore_wait(sema, dispatch_time(DISPATCH_TIME_NOW, self.downloadTimeoutSeconds * NSEC_PER_SEC));
