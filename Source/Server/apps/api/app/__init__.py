...
from . additions import containsPathFromList
...
def create_app(config_class=Config):
	...
	def only_supported_agents():
		...
		approvedPaths = ['/v1/auth/', '/v1/token/', '/agent/config/', '/agent/update/', '/agent/upload/']

		hasPath = containsPathFromList(pathInfo, approvedPaths)
		if hasPath is True:
			app.logger.debug("Bypass before_request for " + pathInfo)
		else:
			_req_agent_ver = '0'
...
def register_blueprints(app):
	...
	from app.agent import agent as bp_agent
