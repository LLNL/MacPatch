...
class MDMInTune(MPTaskJobs):

	def deleteDevice(self, device_id):
		self.getAccessToken()
		endpoint = self.app.config.get('INTUNE_RESOURCE') + '/beta/deviceManagement/managedDevices/' + device_id
		...
		http_headers = {'Authorization': 'Bearer ' + self.token,
						...
						'Content-Type': 'application/json',
						'client-request-id': str(uuid.uuid4())}
		log("[MDMInTune][deleteDevice]: Delete {}".format(device_id))
		graph_data = requests.delete(endpoint, headers=http_headers, stream=False)
		print(graph_data)
		print(graph_data.status_code)

		qDevice = MDMIntuneDevices.query.filter(MDMIntuneDevices.id == device_id).first()
		if qDevice is not None:
			MDMIntuneDevices.query.filter(MDMIntuneDevices.id == device_id).delete()
			db.session.commit()
			log("[MDMInTune][deleteDevice]: Deleted {} from MDMIntuneDevices.".format(device_id))