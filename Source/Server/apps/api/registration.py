...
from app import db
from app.mputil import return_data_for_root_key, read_config_file
from app.model import MpClientsRegistrationSettings, MpSiteKeys
from app.model import AgentConfig, AgentConfigData
from app.model import MpPatchGroup, PatchGroupMembers
from app.model import MpAsusCatalogList
from app.model import MpServer, MpServerList
from app.model import MpSoftwareGroup, MpSoftwareGroupPrivs
from app.model import MpClientGroups, MpClient, MpClientGroupMembers, MpClientTasks
from app.model import MpClientSettings
...
def getDBUserAndPassword():
	print("\nCreate MacPatch database user and password")
	print("----\n")

	_ruser = input("MacPatch database user name[mpdbadm]: ") or "mpdbadm"
	_rPass1 = getpass.getpass("MacPatch database user password: ")
	_rPass2 = getpass.getpass("Verify MacPatch database user password: ")
	...
	print("Please remeber the user name and password and enter them in the config.cfg file.")
	return _ruser,_rPass1

def createDB(dbuser, dbpass):

	_uri = "%s://%s:%s@%s:%s" % (current_app.config['DB_CONNECTOR'], dbuser, dbpass, current_app.config['DB_HOST'], current_app.config['DB_PORT'])
	current_app.config['SQLALCHEMY_DATABASE_URI'] = _uri

	_up = getDBUserAndPassword()
	if _up == None:
		print("User name and password were exited.")
		return None

	_user = _up[0]
	_pass = _up[1]

	sql_str = text(f"create database {current_app.config['DB_NAME_2']}")
	try:
		_res = db.engine.execute(sql_str)
		print('Create Database')
	except exc.SQLAlchemyError as e:
		print("Error creating database. Please make sure the database does not already exist.")
		print("Now exiting.")
		return None

	# Create User
	sql_createUser = text(f"CREATE USER '{_user}'@'%' IDENTIFIED WITH mysql_native_password BY '{_pass}'")
	try:
		_res = db.engine.execute(sql_createUser)
		print('Create User')
	except exc.SQLAlchemyError as e:
		print("Error creating MacPatch user. Please make sure the user does not already exist.")
		print("Now exiting.")
		return None

	# All user to select and show view on sys level
	sql_userRightsSys = text(f"GRANT SELECT, SHOW VIEW on *.* TO '{_user}'@'%'")
	try:
		_res = db.engine.execute(sql_userRightsSys)
		print('Create User Sys Rights')
	except exc.SQLAlchemyError as e:
		print("Error creating MacPatch user sys rights. Please make sure the user does not already exist.")
		print("Now exiting.")
		return None
	
	# Grant All on Database for User
	sql_userRights = text(f"GRANT ALL PRIVILEGES ON {current_app.config['DB_NAME_2']}.* TO '{_user}'@'%' WITH GRANT OPTION")
	try:
		_res = db.engine.execute(sql_userRights)
		print('Create User Database Rights')
	except exc.SQLAlchemyError as e:
		print("Error creating MacPatch user database rights. Please make sure the user does not already exist.")
		print("Now exiting.")
		return None
...
def readDefaultTasks():
	tasks = None
	jFile = os.path.join(basedir, 'console/app/static/json/default_tasks.json')
