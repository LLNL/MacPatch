    ...
    [self connectAndExecuteCommandBlock:^(NSError * connectError)
     {
         if (connectError != nil) {
             ...
             }] readWSData:self.authorization withReply:^(NSError *commandError, NSDictionary *siteDict) {
                 d = [siteDict copy];
    ...
    [self connectAndExecuteCommandBlock:^(NSError * connectError)
     {
         ...
         else
         {
             [[self.helperToolConnection remoteObjectProxyWithErrorHandler:^(NSError * proxyError) {
                 NSLog(@"Error: %@",proxyError.localizedDescription);
             }] writeWSConfig:self.authorization siteConf:aConf launchDConf:lConf withReply:^(NSError * commandError, NSString *licenseKey) {
                 if (commandError != nil) {
...
{
    NSFileManager *fm = [NSFileManager defaultManager];
    if ([fm fileExistsAtPath:TOMCAT_WS] == NO) {
        self.serviceConfText.stringValue = @"Could not locate Tomcat for web services. Please verify that it's installed.";
