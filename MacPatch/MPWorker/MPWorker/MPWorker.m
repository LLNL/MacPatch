...
- (int)runTask:(NSString *)aBinPath binArgs:(NSArray *)aBinArgs environment:(NSString *)env
{
    ...
    [envKeys addObject:@"NSUnbufferedIO"];
    [envKeys addObject:@"COMMAND_LINE_INSTALL"];
    if ([env isEqualToString:@"NA"] == NO && [[env trim] length] > 0)
    {
        ...
        for (id item in l_envArray) {
            ...
            if ([l_envItems count] == 2) {
                ...
                setenv([[l_envItems objectAtIndex:0] UTF8String], [[l_envItems objectAtIndex:1] UTF8String], 1);
                [envKeys addObject:[l_envItems objectAtIndex:0]];
            } else {
    ...
    // Unset ENV variables
    for (NSString *k in envKeys) {
        unsetenv([k UTF8String]);
    }
