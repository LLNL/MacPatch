<cfcomponent output="false" extends="jqGrid">
	...
	<cffunction name="getSUServers" access="remote" returnformat="json">
		...
		<cfloop query="qServers" startrow="#start#" endrow="#end#">
			<cfset arrResults[i] = [#rid#, #catalog_url#, #os_major#, #os_minor#, #proxy#, #iif(active EQ "1",DE("Yes"),DE("No"))#]>
			<cfset i = i + 1>
		</cfloop>
	...
	<cffunction name="editSUServers" access="remote" hint="Add or Edit" returnformat="json" output="no">
		...
		<cfelseif oper EQ "add">
			<cftry>
				<cfquery name="addServer" datasource="#session.dbsource#">
					Insert Into mp_asus_catalogs (listid, catalog_url, os_major, os_minor, proxy, active, catalog_group_name)
					Values (<cfqueryparam value="1">,<cfqueryparam value="#Arguments.catalog_url#">,<cfqueryparam value="#Arguments.os_major#">,
							<cfqueryparam value="#Arguments.os_minor#">,<cfqueryparam value="#Arguments.proxy#">,<cfqueryparam value="#Arguments.active#">,<cfqueryparam value="Default">)
				</cfquery>
				...
				<cfcatch type="any">
					...
					<cflog application="no" file="MPSUServer" type="error" text="[editSUServers][add]: #cfcatch.message# #cfcatch.Detail#" />
				</cfcatch>
			</cftry>
	...
	<cffunction name="updateServerListVersion" access="private">
		...
		<cftry>
			<cfquery name="qGetVer" datasource="#session.dbsource#">
				Select version FROM mp_asus_catalog_list
				WHERE
			...
			<cfif qGetVer.Recordcount GTE 2>
				<cflog application="no" file="MPSUServer" type="error" text="[updateServerListVersion]: mp_server_list contained multiple results. Server list version will not be updated." />
			</cfif>
			<cfcatch type="any">
				<cflog application="no" file="MPSUServer" type="error" text="[updateServerListVersion]: #cfcatch.message# #cfcatch.Detail#" />
			</cfcatch>
		</cftry>
		<cfreturn>
	</cffunction>
