<cfcomponent output="false" extends="jqGrid">
	...
	<cffunction name="getAgents" access="remote" returnformat="json">
		...
		<cftry>
			<cfquery name="qAgents" datasource="#session.dbsource#" result="res">
				select *
		...
		<cfset records = qAgents>
		<cfset start = ((arguments.page-1)*arguments.rows)+1>
		<cfset end = (start-1) + arguments.rows>
		<cfset i = 1>

		<cfloop query="qAgents" startrow="#start#" endrow="#end#">
			<cfset arrResults[i] = [#cuuid#, #cuuid#, #hostname#, #serialno#, #enabled#, #reg_date#]>
			<cfset i = i + 1>
		</cfloop>

		<cfset totalPages = Ceiling(qAgents.recordcount/arguments.rows)>
		<cfset stcReturn = {total=#totalPages#,page=#Arguments.page#,records=#qAgents.recordcount#,rows=#arrResults#}>
		<cfreturn stcReturn>
	</cffunction>
        ...
        <cfelseif oper EQ "del">
            <cftry>
	        	<cfquery name="delPlugin" datasource="#session.dbsource#">
					...
					Where cuuid = <cfqueryparam value="#Arguments.id#">
				</cfquery>
				<cfcatch type="any">
                    <!--- Error, return message --->
                    <cfset strMsgType = "Error">
