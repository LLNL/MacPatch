<cfcomponent output="false" extends="jqGrid">
	...
	<cffunction name="getMPPatches" access="remote" returnformat="json">
		...
		<cfloop query="selUsers" startrow="#start#" endrow="#end#">
			<cfset _active = #iif(active EQ "1",DE("Yes"),DE("No"))#/>
			<cfset arrUsers[i] = [#puuid#, #pkg_url#, #patch_name#, #patch_ver#, #bundle_id#, #patch_severity#, #patch_reboot#, #patch_state#, #_active#, #DateTimeFormat( mdate, "yyyy-MM-dd HH:mm:ss" )#] >
			<cfset i = i + 1>
		</cfloop>
