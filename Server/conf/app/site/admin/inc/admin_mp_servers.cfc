<cfcomponent output="false" extends="jqGrid">
	...
	<cffunction name="getMPServers" access="remote" returnformat="json">
		...
		<cfloop query="qServers" startrow="#start#" endrow="#end#">
			<cfset arrResults[i] = [#rid#, #server#, #port#, #iif(useSSL EQ "1",DE("Yes"),DE("No"))#, #iif(allowSelfSignedCert EQ "1",DE("Yes"),DE("No"))#, #iif(isMaster EQ "1",DE("Yes"),DE("No"))#, #iif(isProxy EQ "1",DE("Yes"),DE("No"))#, #iif(active EQ "1",DE("Yes"),DE("No"))#]>
			<cfset i = i + 1>
		</cfloop>
